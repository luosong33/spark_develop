...
object WindowOpts {
  ...
  def main(args: Array[String]) {
    ...
    val ssc = new StreamingContext(conf, Milliseconds(5000)) // 毫秒
    val lines = ssc.socketTextStream("192.168.92.132", 9999) // 通过nc -lk 9999 远程机拉取数据

    val pairs = lines.flatMap(_.split(" ")).map((_, 1))
    val windowedWordCounts = pairs.reduceByKeyAndWindow((a:Int,b:Int) => (a + b), Seconds(15), Seconds(10)) // Seconds(15)窗口长度，Seconds(10)滑动时间间隔
